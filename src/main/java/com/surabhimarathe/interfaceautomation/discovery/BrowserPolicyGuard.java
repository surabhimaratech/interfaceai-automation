...
public final class BrowserPolicyGuard {
    ...
    public BrowserPolicyGuard(BrowserContext context, ActionPolicy policy, Runnable onDenied) {
        this.policy = policy;
        context.route("**/*", route -> {
            ...
            else { denied = true; onDenied.run(); route.abort(); }
        });
    }
    ...
    public boolean permits(UiAction.Type type, Page page, ElementHandle target) {
        String destination = (String) target.evaluate("""
            e => e.tagName === 'A' ? e.href :
                 (e.form && (e.type === 'submit' || e.type === 'image')) ?
                     (e.hasAttribute('formaction') ? e.formAction : e.form.action) : null
            """);
